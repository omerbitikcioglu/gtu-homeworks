...
void connectionEstablished(client_t cl) {
    ...
    while (connected) {
        ...
        int cl_fd = open(cl.fifo_req_name, O_WRONLY);
        if (cl_fd == -1) {
            perror("Error open client request fifo");
            exit(1);
        }
        sendRequest(cl_fd, req);
        free(req);
        close(cl_fd);
        /* Get response from the server */
        cl_fd = open(cl.fifo_res_name, O_RDONLY);
        if (cl_fd == -1) {
